...
def group_by_period(dates: List[datetime], is_consecutive_fn: Callable) -> List[List[datetime]]:
    """Group dates into consecutive periods using the provided consecutive check function."""
    if not dates:
        return []

    def group_reducer(acc: List[List[datetime]], date: datetime) -> List[List[datetime]]:
        if not acc:
            return [[date]]
...
def calculate_streaks(dates: List[datetime], is_consecutive_fn: Callable) -> Dict[str, int]:
    """Calculate current and longest streaks using provided consecutive check function."""
...
def calculate_completion_rate(dates: List[datetime], start_date: datetime,
                              periodicity: str) -> float:
...
# Main analytics functions that compose the above pure functions
...
def analyze_habits_by_predicate(habits: List[Any], predicate: Callable) -> List[Dict]:
    ...
    return pipe(
        ...
        lambda hs: map(lambda h: {
            "name": h.name,
            "stats": analyze_habit(h)
        }, hs),
        list
    )
...
def analyze_all_habits(habits: List[Any]) -> Dict[str, Stats]:
    ...
    return pipe(
        habits,
        lambda hs: map(lambda h: (h.name, analyze_habit(h)), hs),
        dict
    )
...
def get_habits_by_periodicity(habits: List[Any], periodicity: str) -> List[Dict]:
    ...
    return analyze_habits_by_predicate(
        habits,
        lambda h: h.periodicity == periodicity
    )
